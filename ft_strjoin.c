...
/*   Updated: 2024/03/05 10:50:53 by tbayrakt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char *str_ptr;    // Pointer to the concatenated string
	size_t total_len; // Total length of the concat str
	if (!s1 || !s2)
		return (NULL);
	...
	if (!str_ptr)
		return (NULL);
	// Copy the contents of s1 into the allocated memory
	ft_strlcpy(str_ptr, s1, total_len + 1);
	// Concatenate the contents of s2 to the end of str_ptr
